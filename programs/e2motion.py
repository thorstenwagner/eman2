...
#		self.setWindowTitle("e2motion.py")
		...
		######
		# Folder parameters
		self.vgb0=QGroupBox("Particle Data")
		self.gbl.addWidget(self.vgb0,0,0,1,4)
		
		self.gbl2=QGridLayout(self.vgb0)
		self.wlpath=QLabel("Path: {}".format(self.path))
		...
		self.wlalimaskrot=QLabel("Rot")
		self.vbl1.addWidget(self.wlalimaskrot)
		
		self.wsbalimaskrot=QSpinBox()
		...
		self.wbaligo=QPushButton(unichr(0x2192))
		self.vbl1.addWidget(self.wbaligo)

		self.vbl1.addStretch(5)
		
		# widget for displaying the masked alignment reference
		self.wlalimask=QLabel("<big><pre>Reference</pre></big>")
		...
		self.hbl1a=QHBoxLayout()
		self.gbl.addLayout(self.hbl1a,4,3)
		self.hbl1a.addStretch(5)
		
		self.wbrecalcref=QPushButton("Realign")
		self.hbl1a.addWidget(self.wbrecalcref)
		
		self.wbrrecalcref=QPushButton("Rerefine")
		...
		self.wlroimaskdraw=QLabel("<big><pre>R\nO\nI</pre></big>")
		...
		self.wlarrow1=QLabel(unichr(0x2192))
		...
		self.wllistresult=QLabel("Results")
#		self.wllistresult.setAlignment(Qt.AlignHCenter)
		...
		self.vgb1=QGroupBox("Launch Job")
		self.vbl3.addWidget(self.vgb1)
		
		self.vbl3a=QVBoxLayout()
		...
		self.wbcompute=QPushButton("Compute")
		self.vbl3a.addWidget(self.wbcompute)

		self.wlarrow2=QLabel(unichr(0x2192))
		self.gbl.addWidget(self.wlarrow2,4,7)


		###### Output widgets
		# Class-averages
		self.wlclasses=QLabel("<big><pre>Classes</pre></big>")
		...
		self.wbshowptcl=QPushButton(unichr(0x2193))
...
#		self.wlnptcl.setText("{:1.3f} *sigma, {:1.1f} %".format((self.particles[n2use][0]-self.ptclmean)/self.ptclsigma,100.0*float(n2use)/len(self.particles)))
	...
	def menuFileOpen(self,x):
		if self.particles!=None:
			QMessageBox.warning(None,"Error","%s already contains a stack of particles. A new folder is required to start with a new stack of particles. Rerun without --path option."%self.path)
			return
...
#		mask.process_inplace("threshold.belowtozero",{"minvalue":0.05})				# this limits the range of the mask to improve PCA performance
	...
	def doCompute(self,x=False):
		...
		else : QMessageBox.warning("Unknown mode %d"%mode)
