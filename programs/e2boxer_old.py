...
class SwarmPanel(object):
	...
	def get_widget(self):
		if self.widget == None:
			...
			hbl = QHBoxLayout()
			ptcl_diam_label = QLabel("Particle Diameter:")
			...
			self.ptcl_diam_edit = QLineEdit(str(self.particle_diameter))
			hbl.addWidget(self.ptcl_diam_edit)
			self.clear=QPushButton("Clear Boxes")
			self.clear.setToolTip("Clear boxes generated by the Swarm mode.")
			hbl.addWidget(self.clear)

			vbl.addLayout(hbl)

			self.thrbut = QRadioButton(SwarmBoxer.THRESHOLD)
			self.selbut = QRadioButton(SwarmBoxer.SELECTIVE)
			self.selbut.setChecked(True)
			self.morselbut = QRadioButton(SwarmBoxer.MORESELECTIVE)
			...
			self.groupbox = QGroupBox("Auto Box Method")
			...
			hbl_ww = QHBoxLayout()
			self.view_template=QPushButton( QIcon(get_image_directory() + "pp_boxer_icon.png"),"View Template")
			...
			self.autobox=QPushButton(QIcon(get_image_directory() + "green_boxes.png"),"Autobox")
			...
			hbl_aa = QHBoxLayout()
			self.update_template = QCheckBox("Refresh Template")
			...
			self.advanced_hbl2 = QHBoxLayout()
			self.enable_interactive_threshold  = QCheckBox("Interactive Threshold")
			...
			self.overlap_hbl = QHBoxLayout()
			self.enable_overlap_removal  = QCheckBox("Proximity Threshold")
...
class SwarmTool(SwarmBoxer,EMBoxingTool):
	...
	def icon(self):
		...
		return QIcon(get_image_directory() + "swarm_icon.png")
...
class GaussPanel(object):
	...
	def get_widget(self):
		if self.widget == None:
			...
			hgc = QHBoxLayout()
			gconvheader = QLabel("<b>Parameters of Gauss convolution</b>")
			hgc.addWidget(gconvheader)
			vbl.addLayout(hgc)

			hbl = QHBoxLayout()
			pixel_input_label = QLabel("Input Pixel Size:")
			pixel_input_label.setToolTip("Input pixel size")
			hbl.addWidget(pixel_input_label)

			pixin = gbdb.setdefault('pixel_input',None)
			if pixin == None:
				self.pixel_input_edit = QLineEdit('1.0')
			else:
				self.pixel_input_edit = QLineEdit(str(gbdb['pixel_input']))
			hbl.addWidget(self.pixel_input_edit)

			#vbl.addLayout(hbl)

			pixel_output_label = QLabel("Output Pixel Size:")
			...
			if pixout_cache == None:
				self.pixel_output_edit = QLineEdit('1.0')
			else:
				self.pixel_output_edit = QLineEdit(str(gbdb['pixel_output']))
			hbl.addWidget(self.pixel_output_edit)
			self.new_pixel_output()
			self.new_pixel_input()
			vbl.addLayout(hbl)

			hbl_invcont = QHBoxLayout()
			self.invert_contrast_chk = QCheckBox("Invert Contrast")
			...
			self.use_variance_chk = QCheckBox("Use Variance")
			...
			hbl_gwidth = QHBoxLayout()
			self.gauss_width_slider = QSlider(Qt.Horizontal)
			...
			hbl_gwidth.addWidget(QLabel("Gauss Width Adjust:"))
			self.gauss_width = QLineEdit(self.GW)
			gauss_width_cache = gbdb.setdefault('gauss_width',None)
			if not(gauss_width_cache == None):
				self.gauss_width = QLineEdit(str(gauss_width_cache))
			...
			hbl_thr = QHBoxLayout()
			thr_low_label = QLabel("Threshold Low:")
			hbl_thr.addWidget(thr_low_label)
			self.thr_low_edit = QLineEdit(self.THRNA)
			thrlow_cache = gbdb.setdefault('thr_low',None)
			if not(thrlow_cache == None):
				self.thr_low_edit = QLineEdit(str(thrlow_cache))
			self.new_thr_low()
			hbl_thr.addWidget(self.thr_low_edit)
			thr_hi_label = QLabel("Threshold High:")
			hbl_thr.addWidget(thr_hi_label)

			thrhi_cache = gbdb.setdefault('thr_hi',None)
			if thrhi_cache == None:
				self.thr_hi_edit = QLineEdit(self.THRNA)
			else:
				self.thr_hi_edit = QLineEdit(str(thrhi_cache))
			self.new_thr_hi()
			...
			self.autobox=QPushButton(QIcon(get_image_directory() + "green_boxes.png"),"Run")
			...
			hgctf = QHBoxLayout()
			ctftitle = QLabel("<b>Parameters of CTF estimation</b>")
			hgctf.addWidget(ctftitle)
			vbl.addLayout(hgctf)

			hbl_wscs = QHBoxLayout()

			window_size_label = QLabel("Window size:")
			hbl_wscs.addWidget(window_size_label)
			self.ctf_window_size = QLineEdit(str(gbdb.setdefault('ctf_window',"512")))

			hbl_wscs.addWidget(self.ctf_window_size)

			cs_label = QLabel("Cs:")
			hbl_wscs.addWidget(cs_label)
			self.ctf_cs = QLineEdit(str(gbdb.setdefault('ctf_cs',"2.0")))
			...
			edge_size_label = QLabel("Edge size:")
			hbl_esv.addWidget(edge_size_label)
			self.ctf_edge_size = QLineEdit(str(gbdb.setdefault('ctf_edge',"0")))
			hbl_esv.addWidget(self.ctf_edge_size)

			voltage_label = QLabel("Voltage:")
			hbl_esv.addWidget(voltage_label)
			self.ctf_volt = QLineEdit(str(gbdb.setdefault('ctf_volt',"200.0")))
			...
			self.ctf_overlap_size = QLineEdit(str(gbdb.setdefault('ctf_overlap',"50")))
			hbl_oac.addWidget(self.ctf_overlap_size)

			amplitude_contrast_label = QLabel("Amplitude Contrast:")
			hbl_oac.addWidget(amplitude_contrast_label)
			self.ctf_ampcont = QLineEdit(str(gbdb.setdefault('ctf_ampcont',"10.0")))
			hbl_oac.addWidget(self.ctf_ampcont)

			vbl.addLayout(hbl_oac)

			# cter kboot
			hbl_kboot = QHBoxLayout()
			kboot_label = QLabel("kboot:")
			hbl_kboot.addWidget(kboot_label)

			self.ctf_kboot = QLineEdit(str(gbdb.setdefault('ctf_kboot',"16")))
			hbl_kboot.addWidget(self.ctf_kboot)
			vbl.addLayout(hbl_kboot)

			hbl_estdef = QHBoxLayout()
			hbl_fed = QHBoxLayout()

			fstart_label = QLabel("F_start:")
			hbl_fed.addWidget(fstart_label)
			self.ctf_f_start = QLineEdit(str(gbdb.setdefault('ctf_fstart',"0.020")))
			hbl_fed.addWidget(self.ctf_f_start)

			estimated_defocus_label = QLabel("Estimated defocus:")
			hbl_estdef.addWidget(estimated_defocus_label)
			self.estdef = QLineEdit('')
			hbl_estdef.addWidget(self.estdef)
			vbl.addLayout(hbl_estdef)

			hbl_estdeferr = QHBoxLayout()
			deferr_label = QLabel("Estimated defocus error:")
			hbl_estdeferr.addWidget(deferr_label)
			self.deferr = QLineEdit('')
			hbl_estdeferr.addWidget(self.deferr)
			vbl.addLayout(hbl_estdeferr)

			hbl_astamp = QHBoxLayout()
			astig_amp_label = QLabel("Estimated astigmatism \namplitude:")
			hbl_astamp.addWidget(astig_amp_label)
			self.astamp = QLineEdit('')
			hbl_astamp.addWidget(self.astamp)
			vbl.addLayout(hbl_astamp)

			hbl_astamperr = QHBoxLayout()
			astamperr_label = QLabel("Estimated astigmatism \namplitude error:")
			hbl_astamperr.addWidget(astamperr_label)
			self.astamperr = QLineEdit('')
			hbl_astamperr.addWidget(self.astamperr)
			vbl.addLayout(hbl_astamperr)

			hbl_astagl = QHBoxLayout()
			astig_angle_label = QLabel("Estimated astigmatism \nangle")
			hbl_astagl.addWidget(astig_angle_label)
			self.astagl = QLineEdit('')
			hbl_astagl.addWidget(self.astagl)
			vbl.addLayout(hbl_astagl)

			hbl_astaglerr = QHBoxLayout()
			astaglerr_label = QLabel("Estimated astigmatism \nangle error:")
			hbl_astaglerr.addWidget(astaglerr_label)
			self.astaglerr = QLineEdit('')
			hbl_astaglerr.addWidget(self.astaglerr)
			vbl.addLayout(hbl_astaglerr)

			hbl_ctf_cter = QHBoxLayout()
			self.estimate_ctf_cter =QPushButton("Estimate CTF using CTER")
...
class CTFInspectorWidget(QWidget):
	...
	def update(self):
		QWidget.update(self) #self.paintEvent(None)
		# print "update..."
