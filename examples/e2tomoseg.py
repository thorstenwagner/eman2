...
class TomoSegSliceViewer(QMainWindow):
...
#	def menu_win_average(self) : self.averageviewer.show()
		...
		if options.helixboxer:
			for i,b in enumerate(self.helixboxes):
				...
				if "." in fsp:
					...
					QMessageBox.warning(None,"Error","Please provide a valid image file extension. The numerical sequence will be inserted before the extension.")
					return
	...
	def menu_file_save_boxes_stack(self):
		...
		if fsp[-4:].lower()!=".hdf" :
			QMessageBox.warning(None,"Error","3-D stacks supported only for .hdf files")
			return
...
class TomoSegInspector(QWidget):
...
# 		self.rotatetool.setIcon(QtGui.QIcon(QtGui.QPixmap(rotateicon)))
		...
		self.rotatetool_label = QLabel("Name")
...
# 		self.translatetool.setIcon(QtGui.QIcon(QtGui.QPixmap(crosshairsicon)))
		...
		self.translatetool_label = QLabel("Name")
...
# 		self.ztranslate.setIcon(QtGui.QIcon(QtGui.QPixmap(ztransicon)))
		...
		self.ztranslatetool_label = QLabel("Name")
...
# 		self.scaletool.setIcon(QtGui.QIcon(QtGui.QPixmap(scaleicon)))
		...
		self.scaletool_label = QLabel("Name")
...
# 		self.rulertool.setIcon(QtGui.QIcon(QtGui.QPixmap(rulericon)))
		...
		self.rulertool_label = QLabel("Name")
...
# 		self.selectiontool.setIcon(QtGui.QIcon(QtGui.QPixmap(selectionicon)))
		...
		self.selectiontool_label = QLabel("Name")
...
# 		self.spheretool.setIcon(QtGui.QIcon(QtGui.QPixmap(sphereicon)))
		...
		self.spheretool_label = QLabel("Name")
...
# 			self.updateSceneGraph()
	...
	def getUtilsWidget(self):
		...
		self.open_segmentation_button = QPushButton("Open Segmentation")
	...
	def _on_open_tomogram(self):
		...
		Open a session
		"""
		filename = QFileDialog.getOpenFileName(self, 'Open Tomogram', os.getcwd(), "*.hdf,*.mrc")
		#if filename:
	...
	def _on_open_segmentation(self):
		...
		"""
		# Open the file
		filename = QFileDialog.getOpenFileName(self, 'Open Segmentation', os.getcwd(), "*.hdf,*.xml")
		#if filename:
		#	self.scenegraph().loadSession(filename)
	...
	def _on_save_segmentation(self):
		...
		Save a snapshot of the scene
		"""
		filename = QFileDialog.getSaveFileName(self, 'Save Segmentation', os.getcwd(), "(*.hdf,*.xml)")
