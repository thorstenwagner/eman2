...
#		E2loadappwin("e2rctboxer","filtertab",self.wplot)
	...
	def get_main_tab(self):
		...
		msplitter.setFrameShape(QFrame.StyledPanel)
	...
	def get_filter_tab(self):
		...
		hbl=QHBoxLayout()
		flabel = QLabel("Filter Type:",self)
	...
	def get_processor_tab(self):
		...
		processorwidget = QWidget()
		vboxa = QVBoxLayout()
		
		vbox1 = QVBoxLayout()
		hbl=QHBoxLayout()
		flabel = QLabel("Filter:",self)
		hbl.addWidget(flabel)
		self.processor_combobox = QComboBox()
		...
		hbl2=QHBoxLayout()
		plabel = QLabel("Parameters:",self)
		hbl2.addWidget(plabel)
		self.params_listbox = QLineEdit(str(self.db.getdefault("processorparams",dfl="")), self)
		...
		vbox2 = QVBoxLayout()
		self.processor_but=QPushButton("Filter")
	...
	def get_main(self, layout):
		...
		self.pos_int_validator = QIntValidator(2,5000, self)	#Anything bigger than 5,000 is crazy!!!!
		self.boxsize = QLineEdit(str(self.mediator.boxsize),self)
	...
	def add_boxing_button_group(self,layout):
		...
		grid.addWidget(QLabel("Current Boxing Tool:"),0,0)
		...
		grid.addWidget(QLabel("Quality Score"),2,0)
	...
	def add_controls(self, layout):
		butbox = QHBoxLayout()
		self.write_box_but=QPushButton("Write Boxes")
		butbox.addWidget(self.write_box_but)
		self.write_but=QPushButton("Write Ptcls")
...
class ManualPicker(QWidget):
	def __init__(self, mediator, db):
		...
		self.db=db
		vbl = QVBoxLayout()
		label = QLabel("Manual Picker", self)
		boldfont = QFont()
		boldfont.setBold(True)
		label.setFont(boldfont)
		label.setAlignment(Qt.AlignTop)
		self.clr_but = QPushButton("Clear", self)
		vbl.addWidget(label)
		vbl.addWidget(self.clr_but)
		self.setLayout(vbl)
		
		self.mpsplitter = QSplitter(Qt.Vertical)
		self.mpsplitter.setFrameShape(QFrame.StyledPanel)
...
class PairPickerTool(QWidget):
	def __init__(self, mediator, db):
		...
		vbl = QVBoxLayout()
		label = QLabel("Pair Picker", self)
		boldfont = QFont()
		boldfont.setBold(True)
		label.setFont(boldfont)
		vbl.addWidget(label)

		self.updateboxes_cb = QCheckBox("Update box positions")
		self.updateboxes_cb.setChecked(False)
		vbl.addWidget(self.updateboxes_cb)
		
		self.centertilts_cb = QCheckBox("Center opposite box position")
		self.centertilts_cb.setChecked(False)
		vbl.addWidget(self.centertilts_cb)
		
		hbl = QHBoxLayout()
		slabel = QLabel("Min pairs for xform", self)
		...
		hta = QHBoxLayout()
		tlabel = QLabel("Computed tilt angle", self)
		hta.addWidget(tlabel)
		self.tiltangle = QLineEdit("", self)
		self.tiltangle.setReadOnly(True)
		hta.addWidget(self.tiltangle)
		vbl.addLayout(hta)
		
		htax = QHBoxLayout()
		talabel = QLabel("Computed tilt axis (Y)", self)
		htax.addWidget(talabel)
		self.tiltaxis = QLineEdit("", self)
		self.tiltaxis.setReadOnly(True)
		htax.addWidget(self.tiltaxis)
		vbl.addLayout(htax)
		
		hgamma = QHBoxLayout()
		gammalabel = QLabel("Gamma angle", self)
		...
		hbb = QHBoxLayout()
		self.upboxes_but = QPushButton("Update Boxes", self)
		self.upboxes_but.setEnabled(False)
		hbb.addWidget(self.upboxes_but)
		
		self.centerboxes_but = QPushButton("Center Boxes", self)
		...
		self.ppsplitter = QSplitter(Qt.Vertical)
		self.ppsplitter.setFrameShape(QFrame.StyledPanel)
