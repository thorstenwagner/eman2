...
class EMLightsInspectorBase(object):
	...
	def get_light_tab(self):
		...
		vbl = QVBoxLayout(self.light_tab )
		vbl.setMargin(0)
		vbl.setSpacing(6)
		vbl.setObjectName("Lights")
		
		self.light_manip_check = QCheckBox("Mouse moves lights")
		self.local_viewer_check = QCheckBox("Local light model")
		self.local_viewer_check.setChecked(glGetInteger(GL_LIGHT_MODEL_LOCAL_VIEWER))
		show_lights = QCheckBox("Show lights")
		show_lights.setChecked(self.target().display_lights)
		max_lights_label = QLabel()
		...
		self.light_tab_widget = QTabWidget()
		...
		light_material_tab_widget = QTabWidget()
	...
	def redo_directional_light_list(self):
		...
		for i,l in enumerate(self.gl_lights):
			if glIsEnabled(l):
				pos = glGetLightfv(l,GL_POSITION)
				if pos[3] == 0:
					a = QListWidgetItem("Light "+str(i),self.light_list)
	...
	def new_light(self,point_source=False):
		for i,l in enumerate(self.gl_lights):
			if not glIsEnabled(l):
				...
				if not point_source: 
					a = QListWidgetItem(new_label,self.light_list)
					for item in self.point_light_list.selectedItems(): item.setSelected(False)
						
				else:
					a = QListWidgetItem(new_label,self.point_light_list)
	...
	def get_directional_light_tab(self):
		...
		self.directional_light_widget = QWidget()
		
		vbl = QVBoxLayout(self.directional_light_widget)
		vbl.setMargin(0)
		vbl.setSpacing(6)
		vbl.setObjectName("Lights")
		...
		self.light_list = QListWidget(None)
		...
		self.light_x_dir = QDoubleSpinBox(self)
		...
		self.light_y_dir = QDoubleSpinBox(self)
	...
	def get_pointsource_light_tab(self):
		...
		self.pointsource_light_widget = QWidget()
		
		vbl = QVBoxLayout(self.pointsource_light_widget)
		vbl.setMargin(0)
		vbl.setSpacing(6)
		vbl.setObjectName("Lights")
		...
		self.point_light_list = QListWidget(None)
		...
		self.light_x_pos = QDoubleSpinBox(self)
		...
		self.light_y_pos = QDoubleSpinBox(self)
		...
		self.light_z_pos = QDoubleSpinBox(self)
		...
		self.light_ps_xdir = QDoubleSpinBox(self)
		...
		self.light_ps_ydir = QDoubleSpinBox(self)
...
class EMLightsInspector(QWidget,EMLightsInspectorBase):
	def __init__(self,target) :
		...
		self.wiretog = QPushButton("Wire")
		self.wiretog.setCheckable(1)
		self.vbl2.addWidget(self.wiretog)
		
		self.lighttog = QPushButton("Light")
